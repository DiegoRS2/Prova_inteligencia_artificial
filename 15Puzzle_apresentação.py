...
def update_puzzle(puzzle):
    for i in range(4):
        for j in range(4):
            value = puzzle[i * 4 + j]
            buttons[i][j].config(text=str(value) if value != 0 else "", state=tk.DISABLED)
...
def auto_update_gui():
    if solution_path:
        ...
        root.after(1000, auto_update_gui)
...
buttons = [[tk.Button(frame, width=5, height=2, font=("Helvetica", 12), command=lambda i=i, j=j: move_tile(i, j))
